---
title: HTML DWT Command-line Tool Template Specification
version: 1.0
date_created: 2024-10-06
last_updated: 2025-10-06
owner: isocialPractice Development Team
tags: [tool, command-line, dreamweaver-templates, html-templating, content-management]
---

# HTML DWT Command-line Tool Template Specification

This specification defines the requirements, constraints, and implementation guidelines for a command-line tool that replicates Adobe Dreamweaver's templating system functionality, enabling creation and management of HTML/PHP files based on template files (.dwt, .htm, .html) with editable and non-editable regions. The scope explicitly includes automated synchronization of Dreamweaver-style conditional (optional) regions so TemplateBeginIf/TemplateEndIf blocks stay aligned between template and instance documents.

Adhere to the standards of `html-dwt-template/CODING_STANDARDS.md`.

## 1. Purpose & Scope

This specification provides comprehensive requirements for developing a command-line tool that emulates Adobe Dreamweaver's template system. The command-line tool shall enable developers to:
...
**Intended Audience**: command-line tool developers, web developers familiar with Dreamweaver templates
...
- **REQ-001**: command-line tool shall recognize template files with extensions .dwt, .htm, .html located in Templates/ directory
- **REQ-002**: command-line tool shall support HTML comment-based template syntax identical to Adobe Dreamweaver
- **REQ-003**: command-line tool shall create instance files from templates with proper InstanceBegin/InstanceEnd markers
- **REQ-004**: command-line tool shall preserve editable content when updating instances from modified templates
- **REQ-005**: command-line tool shall prevent editing of protected regions in instance files while allowing full editing of template files
- **REQ-006**: command-line tool shall support nested template hierarchies with parent-child relationships
- **REQ-007**: command-line tool shall provide visual indicators for editable and protected regions
- **REQ-008**: command-line tool shall create automatic backups before template synchronization operations
- **REQ-009**: command-line tool shall synchronize conditional optional regions during updates, keeping show/hide states and linked parameters consistent across instance files.

### Template Syntax Requirements

- **SYN-001**: command-line tool shall recognize TemplateBeginEditable/TemplateEndEditable markers in template files
- **SYN-002**: command-line tool shall convert template markers to InstanceBeginEditable/InstanceEndEditable in instance files
- **SYN-003**: command-line tool shall support TemplateBeginRepeat/TemplateEndRepeat for repeating content sections
- **SYN-004**: command-line tool shall implement TemplateBeginIf/TemplateEndIf for conditional content display
- **SYN-005**: command-line tool shall support TemplateParam declarations with types: text, boolean, color, URL, number
- **SYN-006**: command-line tool shall evaluate template expressions using @@(expression)@@ syntax
- **SYN-007**: command-line tool shall support TemplateInfo codeOutsideHTMLIsLocked attribute for HTML preservation
- **SYN-008**: command-line tool shall apply TemplateBeginIf/TemplateEndIf logic when generating or updating instances, honoring default visibility and parameter linkage defined in the template.
...
- **STR-003**: command-line tool shall maintain proper document-relative link paths when creating instances
- **STR-004**: command-line tool shall support Templates/ directory structure with nested folders
- **STR-005**: Backup files shall be stored in .html-dwt-template-backups/ directory with version control

### Content Preservation Requirements

- **CNT-001**: command-line tool shall preserve all content within editable regions during template updates
- **CNT-002**: command-line tool shall maintain repeating region entries and their content during synchronization
- **CNT-003**: command-line tool shall preserve alternating background colors in repeating table rows using _index expressions
- **CNT-004**: command-line tool shall handle optional region visibility states based on parameter values
- **CNT-005**: command-line tool shall maintain custom attributes in editable tag attributes
- **CNT-006**: command-line tool shall propagate updated optional region expressions and parameter bindings from templates to instances without overwriting user-managed defaults.

### Protection System Requirements

- **PRT-001**: command-line tool shall block editing of protected regions with immediate content restoration
- **PRT-002**: command-line tool shall allow per-file protection toggle while maintaining global protection settings
- **PRT-003**: command-line tool shall provide visual highlighting of protected and editable regions
- **PRT-004**: command-line tool shall create document snapshots for rapid content restoration
- **PRT-005**: command-line tool shall allow unrestricted editing of .dwt template files

### User Interface Requirements

- **UI-001**: command-line tool shall provide commands for creating new pages from templates
- **UI-002**: command-line tool shall offer diff navigation with Previous/Next buttons for template updates
- **UI-003**: command-line tool shall display confirmation dialogs for destructive operations
- **UI-004**: command-line tool shall show progress indicators for long-running operations
- **UI-005**: command-line tool shall provide quick-pick lists for editable region navigation

### Safety & Validation Requirements

- **SAF-001**: command-line tool shall validate template syntax before applying changes
- **SAF-002**: command-line tool shall check for balanced editable region markers
- **SAF-003**: command-line tool shall detect content loss and warn users before proceeding
- **SAF-004**: command-line tool shall implement size ratio checks to prevent content corruption
- **SAF-005**: command-line tool shall provide safety diff views when validation fails
- **SAF-006**: command-line tool shall verify TemplateBeginIf/TemplateEndIf pairs remain balanced and resolvable before applying synchronized updates.

### Performance Requirements

- **PRF-001**: command-line tool shall process template updates incrementally with progress reporting
- **PRF-002**: command-line tool shall optimize file I/O operations using efficient parsing algorithms
- **PRF-003**: command-line tool shall limit document scanning to relevant sections (first 600 characters for header detection)
- **PRF-004**: command-line tool shall implement caching for template instance relationships
- **PRF-005**: command-line tool shall clean up temporary files after operations complete

### Constraints

- **CON-001**: Template files not in Templates/ directory shall not trigger automatic instance detection
- **CON-002**: command-line tool shall not modify backup files or files in .html-dwt-template-backups/ directory
- **CON-003**: Template expressions shall use JavaScript subset syntax only (no full JavaScript execution)
- **CON-004**: command-line tool shall support maximum 10 levels of nested template hierarchy
...
### command-line tool Command Interface
...
- **AC-002**: Given a template file in Templates/ directory, When template is modified and saved, Then command-line tool should offer to update all associated instance files
...
- **AC-009**: The command-line tool shall create automatic backups before synchronization operations complete successfully
...
- **AC-013**: Given template syntax errors, When attempting synchronization, Then command-line tool should display specific error messages and prevent operation
- **AC-014**: Given potential content loss scenario, When safety checks fail, Then command-line tool should show diff view and allow user to review changes
- **AC-015**: Given unbalanced template markers, When validating template, Then command-line tool should report specific marker mismatch errors
...
### Frameworks
- **command-line tool Tester**: For command-line tool-specific functionality testing
...
- **Memory Testing**: command-line tool memory usage during large workspace operations
...
**Adobe Dreamweaver Syntax Compatibility**: The command-line tool replicates Dreamweaver's exact comment-based template syntax to ensure seamless migration of existing template-based sites. This maintains compatibility with legacy workflows while providing modern VS Code editing experience.
...
**Conditional Region Synchronization**: Adobe's optional-region guidance (last updated 2021-04-27) requires TemplateBeginIf blocks to propagate parameter-driven visibility. The command-line tool mirrors that behavior by updating instance visibility defaults and linked parameters whenever conditional logic changes in the template.

### Business Context

Web development teams using Dreamweaver templates require modern development tools while maintaining existing template-based workflows. This command-line tool enables migration to VS Code ecosystem without rebuilding template systems, preserving development investments and team expertise.

### Technical Context

VS Code's command-line tool API provides necessary file system monitoring, document editing, and UI integration capabilities for template management. The command-line tool leverages VS Code's diff viewer, workspace management, and command system to provide integrated template development experience.
...
### command-line tool API Dependencies
...
- **API-003**: Command API - command-line tool command registration and execution
...
- **Handling**: command-line tool should detect and report specific line numbers of unbalanced markers

#### Nested Editable Regions  
- **Edge Case**: TemplateBeginEditable inside another TemplateBeginEditable
- **Handling**: command-line tool should process top-level regions first, handle nested regions in content preservation

#### Empty Template Files
- **Edge Case**: .dwt file with no editable regions
- **Handling**: command-line tool should warn user and allow creation of instances with no editable content

#### Template Parameter Type Mismatches
- **Edge Case**: TemplateParam type="boolean" with non-boolean default value
- **Handling**: command-line tool should validate parameter types and provide default type-appropriate values

#### Large File Processing
- **Edge Case**: Template with thousands of instances across site
- **Handling**: command-line tool should process in batches with progress indication and allow cancellation

#### Circular Template References
- **Edge Case**: Template A references Template B which references Template A
- **Handling**: command-line tool should detect circular references and prevent infinite recursion

## 10. Validation Criteria

### Template Syntax Validation

The command-line tool shall validate template files against the following criteria:
...
2. **Memory Usage**: command-line tool memory consumption remains below 100MB during operations
...
### command-line tool Base Development
- [command-line tool API](https://code.visualstudio.com/api) - Official extension development documentation
- [command-line tool Guidelines](https://code.visualstudio.com/api/references/extension-guidelines) - Best practices for extension development
