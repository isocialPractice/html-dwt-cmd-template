...
import { TextDocument } from '../adapters/types';
...
export function isDreamweaverTemplate(document: TextDocument | { getText(): string }): boolean {
  const text = document.getText();
  return DREAMWEAVER_COMMENT_REGEX.test(text) || CSP_COMMENT_REGEX.test(text);
}

export function isDreamweaverTemplateFile(document: TextDocument | { fileName: string }): boolean {
