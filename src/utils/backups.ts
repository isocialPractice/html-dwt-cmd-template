...
export async function createHtmlBackups(instances: Uri[], templatePath: string, skipBackup: boolean = false): Promise<string> {
  ...
  const backupDir = path.join(siteRoot, '.html-dwt-cmd-template-backups');
  ...
  for (const instanceUri of instances) {
    if (instanceUri.fsPath.includes('.html-dwt-cmd-template-backups')) {
...
export async function restoreHtmlFromBackup(): Promise<void> {
  ...
  if (!lastBackupInfo) {
    await platform.interaction.showMessage('No backup information found. Cannot restore files.', 'error');
    return;
  }
  ...
  if (!fs.existsSync(backupDir)) {
    await platform.interaction.showMessage(`Backup directory not found: ${backupDir}`, 'error');
    return;
  }
  ...
  await platform.interaction.withProgress({
    ...
    await platform.interaction.showMessage('Restore completed from last backup.', 'info');
  });
}
