      ...
      if (safetyIssues.length && !options.suppressSafetyChecks) {
        ...
        if (decision === SHOW) {
          try {
            ...
            const tempDir = path.join(siteRoot, '.html-dwt-cmd-template-temp');
...
export async function updateHtmlBasedOnTemplate(
  ...
  await vscode.window.withProgress({
    ...
    const tempDiffDir = path.join(siteRootForTemp, '.html-dwt-cmd-template-temp');
    ...
    const cleanupTempDirectory = () => {
      try {
        ...
        const hasOpenTempEditors = vscode.window.visibleTextEditors.some(e => e.document.uri.fsPath.includes('.html-dwt-cmd-template-temp'));
